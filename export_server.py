...
function I(){{var loaded=false;try{{var d=JSON.parse(localStorage.getItem('gk_xs'));if(d){{Object.keys(d).forEach(function(k){{S[k]=d[k];}});loaded=true;}}}}catch(e){{}}Object.keys(P).forEach(function(k,i){{if(!loaded||!S[k])S[k]={{a:{{}},s:{{}}}};R(k,i);}});document.getElementById('pvTabs').addEventListener('click',function(e){{var t=e.target.closest('.tb');if(!t)return;document.querySelectorAll('#pvTabs .tb').forEach(function(x){{x.classList.remove('active');}});t.classList.add('active');document.querySelectorAll('.pn').forEach(function(x){{x.classList.remove('active');}});document.getElementById('p-'+t.dataset.p).classList.add('active');var k2=Object.keys(P)[parseInt(t.dataset.p)];R(k2,parseInt(t.dataset.p));updateAS(parseInt(t.dataset.p));}});}}
...
function C(k,qi,oi){{var q=P[k].d[qi],a=S[k].a[qi]||[];if(q.s==='single'||q.s==='judge')a=[oi];else{{var p=a.indexOf(oi);if(p>=0)a.splice(p,1);else a.push(oi);}}S[k].a[qi]=a;R(k,Object.keys(P).indexOf(k));saveST();updateAS(Object.keys(P).indexOf(k));}}
function SB(k,qi){{var q=P[k].d[qi];var ok=q.s==='subjective'?true:(q.a.length===(S[k].a[qi]||[]).length&&q.a.every(function(a){{return(S[k].a[qi]||[]).indexOf(a)>=0;}}));S[k].s[qi]={{o:ok}};R(k,Object.keys(P).indexOf(k));saveST();updateAS(Object.keys(P).indexOf(k));}}
function RST(k){{S[k]={{a:{{}},s:{{}}}};R(k,Object.keys(P).indexOf(k));saveST();updateAS(Object.keys(P).indexOf(k));}}
function saveST(){{try{{var d={{}};Object.keys(S).forEach(function(k){{d[k]={{a:S[k].a,s:S[k].s}};}});localStorage.setItem('gk_xs',JSON.stringify(d));}}catch(e){{}}}}function loadST(){{try{{var d=JSON.parse(localStorage.getItem('gk_xs'));if(d){{Object.keys(d).forEach(function(k){{S[k]=d[k];}});return true;}}}}catch(e){{}}return false;}}function updateAS(idx){{var ag=document.getElementById('asGrid');if(!ag)return;ag.innerHTML='';var k=Object.keys(P)[idx];var p=P[k];for(var i=0;i<p.d.length;i++){{var c='as-num';var s=S[k].s[i];if(s)c+=s.o?' correct':' wrong';(function(i,c){{var d=document.createElement('div');d.className=c;d.textContent=i+1;d.onclick=function(){{document.querySelectorAll('.cd')[i].scrollIntoView({{behavior:'smooth',block:'center'}});}};ag.appendChild(d);}})(i,c);}}}}document.addEventListener('click',function(e){{var op=e.target.closest('.op');if(op&&op.dataset.qi!=null){{var k=Object.keys(P)[parseInt(document.querySelector('.tb.active').dataset.p)];C(k,parseInt(op.dataset.qi),parseInt(op.dataset.oi));return;}}var btn=e.target.closest('button[data-submit]');if(btn){{var k=Object.keys(P)[parseInt(document.querySelector('.tb.active').dataset.p)];SB(k,parseInt(btn.dataset.submit));}}}});
I();setTimeout(function(){{updateAS(0);}},100);'''
