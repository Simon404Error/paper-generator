...
function C(k,qi,oi){{var q=P[k].d[qi],a=S[k].a[qi]||[];if(q.s==='single'||q.s==='judge')a=[oi];else{{var p=a.indexOf(oi);if(p>=0)a.splice(p,1);else a.push(oi);}}S[k].a[qi]=a;R(k,Object.keys(P).indexOf(k));updateAS(Object.keys(P).indexOf(k));}}
function SB(k,qi){{var q=P[k].d[qi];var ok=q.s==='subjective'?true:(q.a.length===(S[k].a[qi]||[]).length&&q.a.every(function(a){{return(S[k].a[qi]||[]).indexOf(a)>=0;}}));S[k].s[qi]={{o:ok}};R(k,Object.keys(P).indexOf(k));updateAS(Object.keys(P).indexOf(k));}}
function RST(k){{S[k]={{a:{{}},s:{{}}}};R(k,Object.keys(P).indexOf(k));updateAS(Object.keys(P).indexOf(k));}}
function updateAS(idx){{var ag=document.getElementById('asGrid');if(!ag)return;ag.innerHTML='';var k=Object.keys(P)[idx];var p=P[k];for(var i=0;i<p.d.length;i++){{var c='as-num';var s=S[k].s[i];if(s)c+=s.o?' correct':' wrong';(function(i,c){{var d=document.createElement('div');d.className=c;d.textContent=i+1;d.onclick=function(){{document.querySelectorAll('.cd')[i].scrollIntoView({{behavior:'smooth',block:'center'}});}};ag.appendChild(d);}})(i,c);}}}}document.addEventListener('click',function(e){{var op=e.target.closest('.op');if(op&&op.dataset.qi!=null){{var k=Object.keys(P)[parseInt(document.querySelector('.tb.active').dataset.p)];C(k,parseInt(op.dataset.qi),parseInt(op.dataset.oi));return;}}var btn=e.target.closest('button[data-submit]');if(btn){{var k=Object.keys(P)[parseInt(document.querySelector('.tb.active').dataset.p)];SB(k,parseInt(btn.dataset.submit));}}}});
